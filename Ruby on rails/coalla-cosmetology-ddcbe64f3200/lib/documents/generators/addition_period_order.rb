module Documents
  module Generators
    class AdditionPeriodOrder
      ...
      def generate
        build_doc('addition_period_order', data)
      end
      ...
      def data
        {
          students_section: ->(report) do
            report.add_section('STUDENTS_SECTION', @grouped_subscriptions) do |s|
              ...
              s.add_field(:course_start) { |gs| format_date_with_points(gs.group.begin_on) }
              s.add_field(:course_end) { |gs| format_date_with_points(gs.group.end_on) }
              s.add_field(:course_duration) { |gs| format_duration(gs.course.hours) }
              s.add_field(:course_name) { |gs| gs.course.name }
              s.add_field(:course_appointment) { |gs| I18n.t("course.appointment.#{gs.course.appointment}") }
            end
          end,
