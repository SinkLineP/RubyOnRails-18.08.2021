$(function () {
  ...
  //
  $(document).on("click", ".button.has_many_remove", function (event) {
    ...
    if (to_remove.hasClass("new_record")) {
      to_remove.remove();
    } else {
  ...
  $(document).on("click", "a.button.has_many_add", function (event) {
    ...
    if (!before_add.isDefaultPrevented()) {
      var index =
        parent.data("has_many_index") || parent.children("fieldset").length - 1;
      ...
      recompute_positions(parent);
      var phone_inputs = document.querySelectorAll(".js-intl-tel");
      window.initPhoneInput(phone_inputs[phone_inputs.length - 1]);
