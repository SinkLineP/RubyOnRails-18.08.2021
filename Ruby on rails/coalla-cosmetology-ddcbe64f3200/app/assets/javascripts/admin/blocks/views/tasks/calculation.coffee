B.CalculationView = Backbone.View.extend
  template: _.template($('#task-calculation-template').html())
  events:
    'change .js-max-attempts-count': 'updateMaxAttemptsCount'
    'change .js-description': 'updateDescription'
    'change .js-answer': 'updateAnswer'
  ...
  updateAnswer: (event)->
    @model.set('answer', event.target.value)
